export function watermark(element, setting) {
  //默认设置
  var defaultSettings = {
    watermark_txt: "text",
  ...
  if (arguments[1].length === 1 && typeof arguments[1] === "object") {
    var src = arguments[1] || {};
    for (var keyS in src) {
      if (
        src[keyS] &&
  ...
  //获取页面最大高度
  //   var page_height =
  //     Math.max(document.body.scrollHeight, document.body.clientHeight) + 650;
