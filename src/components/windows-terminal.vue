<template>
  <div class="app" v-show="isShow" id="windows-terminal">
    <div class="container">
      <header>
        <!-- <h1>Web Terminal</h1>
        <div class="status" :class="statusClass">{{ statusText }}</div> -->
        <div class="icons">
          <div class="close" @click="bus.emit('show-windows-terminal', false)">
            <svg-icon name="close" class="icon"></svg-icon>
          </div>
          <div class="min" @click="isShow = false">
            <svg-icon name="min" class="icon"></svg-icon>
          </div>
...
const connect = () => {
  ...
  ws.value = new WebSocket(
    `${protocol}//121.36.205.18:3003/ws`
  );
...
onMounted(() => {
  ...
  bus.on("hidden-windows-terminal", () => {
    isShow.value = true;
  });
});
...
onUnmounted(() => {
  ...
  disconnect();
  bus.off("hidden-windows-terminal");
});
</script>
